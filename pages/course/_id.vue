      ...
      <div>
        <article class="c-v-pic-wrap" style="height: 357px;">
          <section class="p-h-video-box" id="videoPlay">
            <img height="357px" width="100%px" :src="courseWebVo.cover" :alt="courseWebVo.title" class="dis c-v-pic">
          </section>
        </article>
        <aside class="c-attr-wrap">
          <section class="ml20 mr15">
            ...
            <section class="c-attr-mt" v-else>
              <a @click="createOrders()" href="#" title="立即购买" class="comm-btn c-btn-4">立即购买</a>
            </section>
          </section>
        </aside>
                          ...
                          <li class="lh-menu-stair" v-for="chapter in chapterVideoList" :key="chapter.id">
                            ...
                            <ol class="lh-menu-ol" style="display: block;">
                              <li class="lh-menu-second ml30" v-for="video in chapter.children" :key="video.id">
                                <a :href="'/player/'+video.videoSourceId" target="_blank"  v-if=" isbuy ||Number(courseWebVo.price)===0">
                                  <span class="fr">
                                    <!-- <i class="free-icon vam mr10">开始学习</i> -->
                                    <button class="free-icon vam mr10" height="20px" @click="startLearn">开始学习</button>
                                  </span>
...
export default {
  ...
  //   return course.getFrontCouseInfoById(params.id).then((response) => {
...
data() {
  ...
  methods:{
    ...
    startLearn() {
      // 课程viewcont+1
      // 用户学习的课程记录保存
      this.learnHistory.courseId = this.courseWebVo.id;
      history.addLearnHistory(this.learnHistory)
      console.log("debug 999");
    },
    //获取课程详情
    initCourseInfo() {
      course.getFrontCouseInfoById(this.courseId)
            .then(response => {
